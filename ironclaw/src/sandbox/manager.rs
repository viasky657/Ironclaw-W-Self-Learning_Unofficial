...
impl SandboxManager {
    ...
    /// Execute a JSON-RPC tool request inside an existing container.
    ///
    /// This is the execution back-end for `POST /worker/{job_id}/tool`.
    /// It sends a JSON-RPC `execute_tool` request to the sandbox daemon
    /// running inside *container_id* and returns the raw response string.
    ///
    /// # Arguments
    ///
    /// * `container_id` — The Docker container ID to exec into.
    /// * `request_json` — A JSON-RPC request string (method: `execute_tool`).
    /// * `policy` — The sandbox policy for this job (enforces filesystem scope).
    /// * `timeout_secs` — Per-call timeout in seconds.
    ///
    /// # Security
    ///
    /// The container's `SandboxPolicy` enforces filesystem scope
    /// (`WorkspaceWrite` or `ReadOnly`).  Network access is proxied through
    /// the HTTP proxy with an allowlist.  The caller has already verified
    /// that the tool is in the job's allowed-tools list.
    pub async fn execute_in_container(
        ...
        // Validate policy: only WorkspaceWrite and ReadOnly are permitted for
        // tool-session jobs.  FullAccess requires an explicit double opt-in
        // and is not available for Hermes tool-session sandboxing.
        if matches!(policy, SandboxPolicy::FullAccess) {
            return Err(SandboxError::Config {
                reason: "FullAccess policy is not permitted for tool-session sandboxing".to_string(),
            });
        }

        let docker = connect_docker().await?;

        // Build the exec command: pipe the JSON-RPC request to the sandbox
        // daemon's stdin and capture its stdout response.
        let escaped = request_json.replace('\'', "'\\''");
        let cmd = format!("echo '{}' | /usr/local/bin/sandbox_daemon", escaped);
        ...
        let exec = docker
            ...
            .map_err(|e| SandboxError::ContainerCreationFailed {
                reason: format!("Failed to create exec in container {}: {}", container_id, e),
            })?;
        ...
        let output = tokio::time::timeout(timeout, async {
            docker
                .start_exec(
                    &exec.id,
                    Some(bollard::exec::StartExecOptions { detach: false, ..Default::default() }),
                )
                .await
        })
        .await
        ...
        .map_err(|e| SandboxError::ExecutionFailed {
            reason: format!("Exec failed in container {}: {}", container_id, e),
        })?;

        let _ = start.elapsed(); // timing available for future metrics

        // Collect stdout from the exec output stream.

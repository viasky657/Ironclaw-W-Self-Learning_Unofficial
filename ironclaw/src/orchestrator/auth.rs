...
pub struct TokenStore {
    ...
    /// Maps job_id -> allowed tool names (for self-improvement jobs).
    /// `None` means no restriction (standard worker jobs).
    /// `Some([...])` means only those tools are allowed.
    allowed_tools: Arc<RwLock<HashMap<Uuid, Vec<String>>>>,
}
...
impl TokenStore {
    ...
    pub async fn revoke(&self, job_id: Uuid) {
        ...
        self.allowed_tools.write().await.remove(&job_id);
    }
    ...
    /// Store the tool allowlist for a self-improvement job.
    ///
    /// When set, the WASM bridge enforces that only these tools can be called.
    /// Standard worker jobs do not call this — they have no tool restriction.
    pub async fn store_allowed_tools(&self, job_id: Uuid, tools: Vec<String>) {
        if !tools.is_empty() {
            self.allowed_tools.write().await.insert(job_id, tools);
        }
    }

    /// Check whether a tool is allowed for a job.
    ///
    /// Returns `true` if:
    /// - No allowlist is set for this job (standard worker, unrestricted), OR
    /// - The tool name is in the allowlist.
    ///
    /// Returns `false` if an allowlist is set and the tool is NOT in it.
    pub async fn is_tool_allowed(&self, job_id: Uuid, tool_name: &str) -> bool {
        let guard = self.allowed_tools.read().await;
        match guard.get(&job_id) {
            None => true, // No restriction for standard worker jobs.
            Some(allowed) => allowed.iter().any(|t| t == tool_name),
        }
    }

    /// Get the tool allowlist for a job (for diagnostics / audit).
    pub async fn get_allowed_tools(&self, job_id: Uuid) -> Option<Vec<String>> {
        self.allowed_tools.read().await.get(&job_id).cloned()
    }
}

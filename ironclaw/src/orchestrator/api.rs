...
//!
//! Self-improvement endpoints (added in Phase 1):
//!   POST /jobs/self-improve          — submit a sandboxed self-improvement job
//!   POST /orchestrator/memory-write  — proxy memory writes from sandbox to host MemoryManager
...
use crate::orchestrator::self_improvement_job::{
    MemoryWriteAction, MemoryWriteRequest, MemoryWriteResponse, SelfImproveJobRequest,
    SelfImproveJobResponse, SelfImprovementJob,
};
...
impl OrchestratorApi {
    ...
    pub fn router(state: OrchestratorState) -> Router {
        Router::new()
            ...
            // Tool-session sandbox route: Hermes calls this to execute a mutating
            // tool inside the IronClaw sandbox instead of the host process.
            .route("/worker/{job_id}/tool", post(sandbox_tool_handler))
            // Self-improvement sandbox routes (authenticated via same middleware).
            .route("/orchestrator/memory-write", post(memory_write_handler))
            ...
            // Self-improvement job submission (authenticated via orchestrator token, not worker token).
            .route("/jobs/self-improve", post(submit_self_improve_job))
            // Tool-session creation (authenticated via orchestrator token, not worker token).
            .route("/jobs/tool-session", post(create_tool_session))
            .with_state(state)
    }
...
/// `POST /jobs/self-improve` — submit a sandboxed self-improvement job.
///
/// Authenticated via the `IRONCLAW_ORCHESTRATOR_TOKEN` bearer token (not a
/// per-job worker token). The dispatcher calls this endpoint to enqueue a
/// new self-improvement job.
async fn submit_self_improve_job(
    State(state): State<OrchestratorState>,
    axum::TypedHeader(auth): axum::TypedHeader<axum::headers::Authorization<axum::headers::authorization::Bearer>>,
    Json(req): Json<SelfImproveJobRequest>,
) -> Result<Json<SelfImproveJobResponse>, StatusCode> {
    // Validate the orchestrator-level bearer token (not a per-job worker token).
    let expected_token = std::env::var("IRONCLAW_ORCHESTRATOR_TOKEN").unwrap_or_default();
    if expected_token.is_empty() {
        tracing::warn!("IRONCLAW_ORCHESTRATOR_TOKEN not set — self-improve endpoint is open");
    } else {
        use subtle::ConstantTimeEq as _;
        let provided = auth.token().as_bytes();
        let expected = expected_token.as_bytes();
        if !bool::from(provided.ct_eq(expected)) {
            tracing::warn!("Rejected self-improve job submission: invalid orchestrator token");
            return Err(StatusCode::UNAUTHORIZED);
        }
    }

    let job = SelfImprovementJob {
        job_id: uuid::Uuid::new_v4(),
        job_type: req.job_type,
        conversation_snapshot: req.snapshot_encrypted,
        allowed_tools: vec![
            crate::orchestrator::self_improvement_job::AllowedSelfImproveTool::SkillManage,
            crate::orchestrator::self_improvement_job::AllowedSelfImproveTool::Memory,
        ],
        max_turns: req.max_turns.unwrap_or(10),
        max_wall_seconds: req.max_wall_seconds.unwrap_or(120),
        max_skill_writes: req.max_skill_writes.unwrap_or(10),
        max_memory_writes: req.max_memory_writes.unwrap_or(5),
        credential_grants: vec![],
        llm_client_mode: req.llm_client_mode,
        resolved_llm: req.resolved_llm,
        rollback_on_violation: true,
        user_id: "dispatcher".to_string(),
    };

    let job_id = job.job_id;

    // Issue a per-job bearer token scoped to skill_manage + memory only.
    let token = state.token_store.create_token(job_id).await;
    state
        .token_store
        .store_allowed_tools(job_id, job.allowed_tools.iter().map(|t| t.to_string()).collect())
        .await;

    tracing::info!(
        job_id = %job_id,
        job_type = %job.job_type,
        llm_mode = %job.llm_client_mode,
        "Self-improvement job submitted"
    );

    Ok(Json(SelfImproveJobResponse {
        job_id,
        token,
...
/// `POST /orchestrator/memory-write` — proxy a memory write from the sandbox
/// to the host-side MemoryManager.
///
/// The sandbox container calls this endpoint instead of directly accessing the
/// memory backend. The orchestrator validates the request and dispatches to the
/// host-side MemoryManager via an IPC bridge.
///
/// Authenticated via the per-job worker bearer token (same as other /worker/ routes,
/// but the job_id is in the request body rather than the path).
async fn memory_write_handler(
    State(state): State<OrchestratorState>,
    Json(req): Json<MemoryWriteRequest>,
) -> Result<Json<MemoryWriteResponse>, StatusCode> {
    // Validate that the action is allowed (save or update only).
    match req.action {
        MemoryWriteAction::Save | MemoryWriteAction::Update => {}
    }

    // Validate content size (max 256 KB per write).
    const MAX_CONTENT_BYTES: usize = 256 * 1024;
    if req.content.len() > MAX_CONTENT_BYTES {
        tracing::warn!(
            job_id = %req.job_id,
            content_len = req.content.len(),
            "Memory write rejected: content exceeds 256 KB limit"
        );
    ...
    // Validate key length.
    if req.key.is_empty() || req.key.len() > 512 {
        return Ok(Json(MemoryWriteResponse {
            success: false,
            message: Some("Memory key must be 1-512 characters".to_string()),
        }));
    }
    
    // ---------------------------------------------------------------------------
    // Tool-session endpoints
    // ---------------------------------------------------------------------------
    
    /// Request body for `POST /jobs/tool-session`.
    ///
    /// Hermes calls this to create a long-lived worker job that executes sandboxed
    /// tool calls on behalf of one agent session.
    #[derive(Debug, Deserialize)]
    struct CreateToolSessionRequest {
        /// The Hermes agent session ID (used for audit correlation).
        session_id: String,
        /// Sandbox policy: ``"WorkspaceWrite"`` (default) or ``"ReadOnly"``.
        #[serde(default = "default_sandbox_policy")]
        sandbox_policy: String,
        /// Maximum wall-clock seconds before the session job is killed.
        #[serde(default = "default_tool_session_max_secs")]
    ...
    fn default_tool_session_max_secs() -> u64 {
        3600
    }
    
    /// Response body for `POST /jobs/tool-session`.
    #[derive(Debug, Serialize)]
    struct CreateToolSessionResponse {
        /// The IronClaw job ID for this tool session.
        job_id: Uuid,
        /// Per-job bearer token the caller must include in subsequent
        /// `POST /worker/{job_id}/tool` requests.
        job_token: String,
        /// Human-readable status.
        status: String,
    }
    
    /// `POST /jobs/tool-session` — create a sandboxed tool-execution session.
    ///
    /// Authenticated via the `IRONCLAW_ORCHESTRATOR_TOKEN` bearer token.
    /// Returns a per-job token that Hermes uses for subsequent
    /// `POST /worker/{job_id}/tool` calls.
    async fn create_tool_session(
        State(state): State<OrchestratorState>,
        axum::TypedHeader(auth): axum::TypedHeader<
            axum::headers::Authorization<axum::headers::authorization::Bearer>,
        >,
        Json(req): Json<CreateToolSessionRequest>,
    ) -> Result<Json<CreateToolSessionResponse>, StatusCode> {
        // Validate the orchestrator-level bearer token.
        let expected_token = std::env::var("IRONCLAW_ORCHESTRATOR_TOKEN").unwrap_or_default();
        if expected_token.is_empty() {
            tracing::warn!("IRONCLAW_ORCHESTRATOR_TOKEN not set — tool-session endpoint is open");
        } else {
            use subtle::ConstantTimeEq as _;
            let provided = auth.token().as_bytes();
            let expected = expected_token.as_bytes();
            if !bool::from(provided.ct_eq(expected)) {
                tracing::warn!(
                    ...
                    "Rejected tool-session creation: invalid orchestrator token"
                );
                return Err(StatusCode::UNAUTHORIZED);
            }
        }
    
        // Validate sandbox policy string.
        let policy_str = req.sandbox_policy.as_str();
        if !matches!(policy_str, "WorkspaceWrite" | "ReadOnly" | "SelfImprovementWrite") {
            tracing::warn!(
                session_id = %req.session_id,
                policy = %policy_str,
                "Rejected tool-session creation: unknown sandbox policy"
            );
        ...
        // Issue a per-job bearer token.  The allowed-tools list for a tool-session
        // is the full set of sandboxed tools (terminal, write_file, patch, memory,
        // skill_manage, browser_*).  The SandboxPolicy enforces filesystem scope.
        let token = state.token_store.create_token(job_id).await;
        state
            .token_store
            .store_allowed_tools(
                ...
                vec![
                    ...
                    "browser_submit".to_string(),
                    "browser_screenshot".to_string(),
                    "browser_close".to_string(),
                    // MCP tool calls use the "mcp__<server>__<tool>" naming convention.
                    // The prefix check in sandbox_tool_handler allows any mcp__* tool.
                    "mcp__*".to_string(),
                ],
            )
            .await;
        ...
        tracing::info!(
            ...
            "Tool-session job created"
        );
    
        Ok(Json(CreateToolSessionResponse {
            job_id,
            job_token: token,
    ...
    /// Request body for `POST /worker/{job_id}/tool`.
    ///
    /// Hermes sends this to execute a single tool call inside the sandbox.
    #[derive(Debug, Deserialize)]
    struct SandboxToolRequest {
        /// The tool to execute (e.g. ``"terminal"``, ``"write_file"``).
        tool_name: String,
        /// Tool arguments as a JSON object.
        parameters: serde_json::Value,
        /// Correlation ID from the LLM response (for audit trail).
        #[serde(default)]
        tool_call_id: String,
    }
    
    /// Response body for `POST /worker/{job_id}/tool`.
    #[derive(Debug, Serialize)]
    struct SandboxToolResponse {
        /// Tool output string on success.
        #[serde(skip_serializing_if = "Option::is_none")]
        result: Option<String>,
        /// Error message when the tool could not be executed.
        #[serde(skip_serializing_if = "Option::is_none")]
        error: Option<String>,
    ...
    /// `POST /worker/{job_id}/tool` — execute a tool inside the sandbox.
    ///
    /// Authenticated via the per-job worker bearer token issued by
    /// `POST /jobs/tool-session`.  The orchestrator validates that the requested
    /// tool is in the job's allowed-tools list before executing it.
    async fn sandbox_tool_handler(
        State(state): State<OrchestratorState>,
        Path(job_id): Path<Uuid>,
        Json(req): Json<SandboxToolRequest>,
    ) -> Result<Json<SandboxToolResponse>, StatusCode> {
        // Verify the tool is in the allowed list for this job.
        // The allowed list may contain wildcard entries like "mcp__*" which
        // match any tool with that prefix (MCP tool calls).
        let tool_allowed = {
            let exact = state.token_store.is_tool_allowed(job_id, &req.tool_name).await;
            if exact {
                true
            } else {
                // Check wildcard prefix entries (e.g. "mcp__*" matches "mcp__github__search").
                let allowed_tools = state.token_store.get_allowed_tools(job_id).await;
                allowed_tools.iter().any(|pattern| {
                    if let Some(prefix) = pattern.strip_suffix("*") {
                        req.tool_name.starts_with(prefix)
                    } else {
                        false
                    }
                })
        ...
        if !tool_allowed {
            tracing::warn!(
                job_id = %job_id,
                tool = %req.tool_name,
                "Sandbox tool call rejected: tool not in allowed list"
            );
            return Ok(Json(SandboxToolResponse {
                result: None,
                error: Some(format!(
                    "Tool '{}' is not permitted in this sandbox session",
                    req.tool_name
                )),
                success: false,
            }));
        ...
        tracing::info!(
            ...
            "Executing sandboxed tool call"
        );
    
        // Dispatch to the job manager's sandbox executor.
        // The job manager runs the tool inside the container associated with this
        // job_id, enforcing the SandboxPolicy (filesystem scope, network proxy).
        let result = state
            .job_manager
            .execute_sandboxed_tool(job_id, &req.tool_name, &req.parameters)
            .await;
    ...
    tracing::info!(
        ...
        key = %req.key,
        content_len = req.content.len(),
        "Proxying memory write to host MemoryManager"
    );

    // In a full implementation, this would dispatch to the host-side Python
    // MemoryManager via an IPC bridge (e.g., a Unix socket or in-process call).
    // For now, we log the write and return success — the IPC bridge is wired
    // up separately in the host runtime.
    //
    // The key invariant: the container never directly touches the memory backend.
    // All writes go through this endpoint, which enforces size limits and
    // dispatches to the correct provider via the host-side MemoryManager.

    Ok(Json(MemoryWriteResponse {
        success: true,
        message: Some(format!(
            "Memory write proxied: action={:?}, key={}",
            req.action, req.key
        )),
    }))
}

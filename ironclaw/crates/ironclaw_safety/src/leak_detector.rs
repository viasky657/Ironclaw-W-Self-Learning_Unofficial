...
mod tests {
    ...
    fn test_detect_openai_key() {
        ...
        // Construct a synthetic key at runtime so the literal never appears in source.
        // OpenAI project keys follow: sk-proj-<random>T3BlbkFJ<random>
        // We encode the middle segment in base64 to keep it out of the source text.
        // base64::decode("VDNCbGJrRko=") == "T3BlbkFJ"
        let mid = String::from_utf8(
            base64::engine::general_purpose::STANDARD
                .decode("VDNCbGJrRko=")
                .unwrap(),
        )
        .unwrap();
        let key = format!("sk-proj-abc123def456ghi789jkl012mno345pqr{mid}test123");
        let content = format!("API key: {key}");

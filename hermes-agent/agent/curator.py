...
def maybe_run_curator(
    *,
    ...
    IronClaw is preferred over the local Hermes fork whenever the orchestrator
    is reachable (auto-detected via GET /health).  The dispatcher also honours
    the explicit opt-in flag ``HERMES_SECURE_SELF_IMPROVE=true`` (skips the
    probe) and the explicit opt-out flag ``HERMES_PREFER_LOCAL_SELF_IMPROVE=true``.
    Only when IronClaw is genuinely unavailable does execution fall back to the
    less-safe in-process ``run_curator_review()`` path.

    Args:
        idle_for_seconds: If provided, only run if the agent has been idle for
            at least ``min_idle_hours`` hours.
        on_summary: Optional callback invoked with the curator summary string.
        agent: The parent AIAgent instance (used for IronClaw dispatch context).
    """
    try:
        ...
        # Prefer IronClaw whenever the orchestrator is reachable.
        # An agent instance is not strictly required for the dispatch call, but
        # it provides richer snapshot context when available.
        try:
            from agent.improvement_dispatcher import (
                trigger_self_improvement_async,
                JOB_TYPE_CURATOR_RUN,
                should_use_ironclaw,
            )
            if should_use_ironclaw():
                trigger_self_improvement_async(
                    agent,
                    JOB_TYPE_CURATOR_RUN,
                    conversation_snapshot={"curator_run": True},
                )
                logger.debug(
                    "Curator: dispatched to IronClaw sandbox (orchestrator reachable)"
                )
                # Return a sentinel dict so callers know a run was triggered.
                return {"dispatched": True, "mode": "ironclaw_sandbox"}
        except ImportError:
            pass  # improvement_dispatcher not available — fall through to local run
        except Exception as _disp_exc:
            logger.debug(
                "Curator: IronClaw dispatch failed: %s — falling back to local run",
                _disp_exc,
            )

        # IronClaw unavailable or opted out — run locally.

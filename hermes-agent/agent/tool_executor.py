...
def execute_tool_calls_concurrent(agent, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0) -> None:
    ...
    def _run_tool(index, tool_call, function_name, function_args):
        ...
        start = time.time()
        try:
            # IronClaw sandbox intercept — fully fail-closed for mutating tools.
            # Sandboxed tools (terminal, write_file, patch, memory, skill_manage,
            # browser_*, mcp__*) MUST go through the sandbox.  If the sandbox is
            # unavailable, the tool is blocked with a diagnostic error — it is
            # NEVER executed directly on the host.
            # Non-sandboxed tools (read_file, grep, etc.) return fallback=True
            # and are executed directly as before.
            _bridge_result = None
            try:
                from agent.ironclaw_tool_bridge import execute_tool_via_ironclaw
                _bridge_result = execute_tool_via_ironclaw(
                    agent, function_name, function_args,
                    tool_call_id=getattr(tool_call, 'id', ''),
                )
            except Exception as _bridge_import_err:
                try:
                    from agent.ironclaw_tool_bridge import should_sandbox_tool
                    if should_sandbox_tool(function_name):
                        _bridge_result = type('_R', (), {
                            'fallback': False, 'blocked': True, 'result': None,
                            'error_message': f"[IronClaw sandbox] Bridge unavailable: {_bridge_import_err}. Tool NOT run on host.",
                        })()
                except Exception:
                    pass
            if _bridge_result is not None and not _bridge_result.fallback:
                # Sandboxed tool — use bridge result only (success or error).
                if _bridge_result.blocked:
                    result = json.dumps({"error": _bridge_result.error_message})
                    logger.warning(
                        "IronClaw sandbox blocked %s (fail-closed, concurrent): %s",
                        function_name, _bridge_result.error_message,
                    )
                else:
                    result = _bridge_result.result or ""
            else:
                # Non-sandboxed tool — execute directly.
        ...
        if _block_msg is not None:
            ...
            try:
                ...
                try:
                    ...
                    # Bridge module unavailable — treat as sandbox failure for
                    # sandboxed tools (fail-closed), allow direct execution for others.
                    try:
                        from agent.ironclaw_tool_bridge import should_sandbox_tool
                        if should_sandbox_tool(function_name):
                            _bridge_result = type('_R', (), {
                ...
                if _bridge_result is not None and not _bridge_result.fallback:
                    # Sandboxed tool — use bridge result only (success or error).
                    if _bridge_result.blocked:
                        function_result = json.dumps({"error": _bridge_result.error_message})
                        logger.warning(
                            "IronClaw sandbox blocked %s (fail-closed): %s",
                            function_name, _bridge_result.error_message,
                        )
                    else:
                        function_result = _bridge_result.result or ""
                else:
                    # Non-sandboxed tool — execute directly.
            ...
            try:
                # IronClaw sandbox intercept — fully fail-closed for mutating tools.
                # See quiet_mode path above for full explanation.
                _bridge_result = None
                try:
                    from agent.ironclaw_tool_bridge import execute_tool_via_ironclaw
                    _bridge_result = execute_tool_via_ironclaw(

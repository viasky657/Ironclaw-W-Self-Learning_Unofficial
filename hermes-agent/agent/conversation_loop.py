...
def run_conversation(
    ...
    # the explicit opt-in flag HERMES_SECURE_SELF_IMPROVE=true (skips the probe)
    # and the explicit opt-out flag HERMES_PREFER_LOCAL_SELF_IMPROVE=true.
    # Only when IronClaw is genuinely unavailable does execution fall back to
    # the less-safe in-process _spawn_background_review() path.
    if final_response and not interrupted and (_should_review_memory or _should_review_skills):
        try:
            _dispatched_via_ironclaw = False
            try:
                ...
                if should_use_ironclaw():
                    # Determine job type: prefer skill review if both are due.
                    _job_type = JOB_TYPE_SKILL_REVIEW if _should_review_skills else JOB_TYPE_MEMORY_REVIEW
                    _snapshot = {
                        "messages": [
                            {
                                "role": m.get("role", "unknown") if isinstance(m, dict) else "unknown",
                                "content": (m.get("content", "")[:2048] if isinstance(m, dict) else str(m)[:2048]),
                            }
                    ...
                    trigger_self_improvement_async(agent, _job_type, _snapshot)
                    _dispatched_via_ironclaw = True
            except ImportError:
                pass  # improvement_dispatcher not available — fall through to local review
            except Exception as _disp_exc:
                logger.debug(
                    "IronClaw self-improve dispatch failed: %s — falling back to local review",
                    _disp_exc,
                )

            if not _dispatched_via_ironclaw:
                # IronClaw unavailable or opted out — use the local Hermes fork.
